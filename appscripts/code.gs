...
/**
 * Combined init endpoint — returns transactions + summary + dropdown options
 * in a single call. Reads the sheet once to avoid redundant I/O.
 * Params: ?action=init&fy=2026
 */
function getInit(e) {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_NAME);
  if (!sheet) return jsonResponse({ status: "error", message: "Sheet not found" });
  ...
  if (lastRow <= 1) {
    return jsonResponse({
      status: "ok",
      transactions: [],
      summary: { fy, fyTotal: 0, personal: 0, work: 0, avgPerMonth: 0, distinctMonths: 0, byMonth: {}, byMode: {}, bySubcat: {} },
  ...
  // Build transactions array (all transactions, no FY filter — client filters)
  const transactions = [];
  // Build summary (FY-filtered)
  let fyTotal = 0, personal = 0, work = 0;
  const byMonth = {}, byMode = {}, bySubcat = {};
  ...
  data.forEach((row, i) => {
    ...
    // Summary — only for the requested FY
    if (tx.fyStart === fy) {
      ...
      byMonth[tx.month] = (byMonth[tx.month] || 0) + amount;
      if (tx.mode) byMode[tx.mode] = (byMode[tx.mode] || 0) + amount;
      if (tx.subcategory) bySubcat[tx.subcategory] = (bySubcat[tx.subcategory] || 0) + amount;
    }
  });
